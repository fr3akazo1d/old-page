...
read_categories() {
  ...
  local _categories="$(echo "$_yaml" | /snap/bin/yq r - "categories.*")"
  local _category="$(echo "$_yaml" | /snap/bin/yq r - "category")"
...
read_tags() {
  local _yaml="$(_read_yaml "$1")"
  local _tags="$(echo "$_yaml" | /snap/bin/yq r - "tags.*")"
  local _tag="$(echo "$_yaml" | /snap/bin/yq r - "tag")"
